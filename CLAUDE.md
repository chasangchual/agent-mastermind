...
`LANGFUSE_*`/`MASTERMIND_*`/provider env vars can be supplied via a `.env` file for local dev (gitignored; see `.env.example`) or real environment variables in production — `cli.py::main()` calls `python-dotenv`'s `load_dotenv()` first, before anything else touches `os.environ`, and `load_dotenv()` never overrides a variable the real environment already set.
